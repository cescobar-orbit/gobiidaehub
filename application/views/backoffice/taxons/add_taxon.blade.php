...
<br/>
<h3>{{ Lang::line('lbl_taxons.taxons')->get($lang) }} &nbsp; <small>{{ Lang::line('lbl_taxons.create_taxon')->get($lang) }}</small></h3>
    ...
    <div class="container-fluid">
      ...
      {{ Form::open('', 'post', array('id' => 'frm', 'class' => 'form-horizontal well')) }}
               ...
               <label class="control-label" for="Startdate">{{ Lang::line('lbl_taxons.date')->get($lang) }}</label>
                <div class="controls">         
				  <div id="datetimepicker1" class="input-append"> 
				    ...
				    <input type="text" id="updated" name="updated" data-format="yyyy-MM-dd" value="{{ $taxon->updated }}"></input>  
				  </div>                 
			   </div>
             </div>  
...
$(document).ready(function(e) {
	 ...
	 $('#frm').validate(
	  {
		rules: {
			    taxonparentid: { required: true},
			    taxonname: { required: true },
			    taxontype: { required: true },
			   },
